...
export function WorkflowActions({ onWorkflowStarted }: WorkflowActionsProps) {
  ...
  const [analysisType, setAnalysisType] = useState<string>('complete_analysis');


  const startCompleteAnalysis = async (params: any = {}) => {
    setLoading(true);
    try {
      // Ejecutar los 3 tipos de análisis en secuencia
      const analysisTypes = [
        { type: 'trend_analysis', name: 'Análisis de Tendencias' },
        { type: 'monthly_summary', name: 'Resumen de Período' },
        { type: 'analyze_high_risk', name: 'Detección de Alto Riesgo' }
      ];
      ...
      for (const analysis of analysisTypes) {
        try {
          ...
          let analysisParams = params;
          if (analysis.type === 'monthly_summary') {
            // Para resumen mensual, solo usar el mes/año de inicio
            analysisParams = { year: params.start_year, month: params.start_month };
          } else if (analysis.type === 'analyze_high_risk') {
            // Para alto riesgo, usar threshold
            analysisParams = { threshold: 50, limit: 50 };
          }

          // Crear y ejecutar workflow directamente sin usar startWorkflow
          ...
          const agent = agentMap[analysis.type] || 'insight_reporting';

          const workflowRequest = {
            workflow_name: `${analysis.type}_${Date.now()}`,
            tasks: [{
          ...
          if (response.ok) {
            ...
            await fetch(`http://localhost:8001/api/v1/workflows/${data.workflow_id}/execute`, {
              method: 'POST'
            });
            ...
            notifications.show({
              title: `✅ ${analysis.name} Iniciado`,
              message: `Progreso: ${completedCount}/${totalCount}`,
              color: 'blue'
            });
          }
          ...
          console.error(`Error en ${analysis.name}:`, error);
          notifications.show({
            title: `⚠️ Error en ${analysis.name}`,
            message: 'Continuando con el siguiente análisis...',
            color: 'orange'
          });
        }
      }
      ...
      notifications.show({
        title: '🎉 Análisis Completo Iniciado',
        message: `Se iniciaron ${completedCount} de ${totalCount} análisis`,
        color: 'green'
      });
      ...
      notifications.show({
        title: '❌ Error',
        message: 'Hubo un error durante el análisis completo',
        color: 'red'
      });
    } finally {
      setLoading(false);
        ...
        <>
          ...
          {workflowType === 'custom_period' && (
            ...
            <Select
              ...
              data={[
                { value: 'complete_analysis', label: '🎯 Análisis Completo - Todos los tipos combinados' },
            ...
            {(analysisType === 'trend_analysis' || analysisType === 'complete_analysis') && (
              <Group grow>
                <NumberInput
                  label="Año Fin"
                  min={2020}
            ...
            <Paper p="sm" style={{ backgroundColor: '#e7f5ff', borderLeft: '3px solid #228be6' }}>
              <Text size="sm" fw={500}>
                {analysisType === 'complete_analysis' 
                  ? `🎯 Se ejecutarán 3 análisis: Tendencias (${startMonth}/${startYear}-${endMonth}/${endYear}), Resumen (${startMonth}/${startYear}), y Alto Riesgo`
            ...
            <Button 
              ...
              loading={loading}
              onClick={() => {
                if (analysisType === 'complete_analysis') {
                  // Análisis completo: ejecutar los 3 tipos
                  const params = { 
            ...
            >
              {analysisType === 'complete_analysis' 
                ? '🎯 Iniciar Análisis Completo (3 tipos)'
