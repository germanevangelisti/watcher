...
class MencionResponse(BaseModel):
    ...
    jurisdiccion_id: int
    tipo_mencion: str
    fragmento_texto: str | None
    extra_data: dict | None

    # Datos relacionados
    boletin_fecha: date | None
    boletin_filename: str | None
    jurisdiccion_nombre: str | None
...
class ProcesamientoRequest(BaseModel):
    ...
    fecha_desde: str | None = Field(None, description="Filtrar desde fecha (YYYY-MM-DD)")
    fecha_hasta: str | None = Field(None, description="Filtrar hasta fecha (YYYY-MM-DD)")
...
async def listar_menciones(
    ...
    boletin_id: int | None = Query(None, description="Filtrar por boletín"),
    tipo_mencion: str | None = Query(None, description="Filtrar por tipo de mención"),
