...
async def download_single_boletin(
    ...
    """
    Descarga un boletín específico con organización automática
    """
    y_short = int(str(year)[2:])
    url = BASE_URL.format(
        year=year,
        y_short=y_short,
        month=month,
    ...
    filename = f"{year}{month:02d}{day:02d}_{section}_Secc.pdf"
    
    # Usar estructura organizada año/mes
    filepath = get_boletin_path(year, month, filename)
    
    # Crear directorio si no existe
    filepath.parent.mkdir(parents=True, exist_ok=True)
    
    # Si ya existe, verificar integridad
    if filepath.exists():
        file_size = filepath.stat().st_size
        # Si tiene tamaño razonable (>10KB), considerarlo válido
        if file_size > 10240:
            return {
                "filename": filename,
                "status": "exists",
                "size": file_size,
                "path": str(filepath.relative_to(BOLETINES_BASE_DIR))
            }
        else:
            logger.warning(f"Archivo {filename} existe pero es muy pequeño ({file_size} bytes), re-descargando")
            filepath.unlink()  # Eliminar archivo corrupto
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200 and response.headers.get('Content-Type', '').startswith('application/pdf'):
                ...
                logger.warning(f"❌ No disponible: {filename} (status {response.status_code})")
                return {
                    "filename": filename,
...
async def download_boletines_task(
    ...
    """
    Tarea en background para descargar boletines con organización automática
    """
    ...
    try:
        for single_date in daterange(start_date, end_date):
            ...
            for section in sections:
                current_filename = f"{single_date.year}{single_date.month:02d}{single_date.day:02d}_{section}_Secc.pdf"
                
                # Actualizar estado
                download_status[task_id].current_file = current_filename
                
                # Descargar con delay random para evitar rate limiting
                await asyncio.sleep(random.uniform(1.0, 2.0))
                
                result = await download_single_boletin(
                ...
                if result['status'] in ['downloaded', 'exists']:
                    ...
                    download_status[task_id].errors.append(
                        f"{result['filename']}: {result.get('error', 'unknown')}"
                    )
