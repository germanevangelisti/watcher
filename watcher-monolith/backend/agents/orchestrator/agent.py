...
class AgentOrchestrator:
    ...
    async def create_workflow(self, 
        ...
        if self.persist_enabled:
            self._persist_workflow(workflow, task_definitions)
    ...
    async def execute_workflow(self, workflow_id: str) -> WorkflowState:
        ...
        self._persist_log(workflow.workflow_id, "Iniciando ejecución del workflow")
        ...
        try:
            ...
            if workflow.is_completed():
                ...
                self._persist_workflow_result(workflow)
                self._persist_log(workflow.workflow_id, "Workflow completado exitosamente")
                ...
                self._persist_log(workflow.workflow_id, "Workflow falló - hay tareas con errores", level="error")
                ...
                self._persist_log(workflow.workflow_id, "Workflow esperando aprobación", level="warning")
            ...
            self._update_workflow_status(workflow)
            self._persist_log(workflow.workflow_id, f"Error en ejecución: {str(e)}", level="error")
    ...
    async def _execute_task(self, workflow: WorkflowState, 
        ...
        try:
            ...
            # Persistir resultado
            self._persist_task_result(workflow.workflow_id, task.task_id, result, "completed")
            self._persist_log(workflow.workflow_id, f"Tarea {task.task_id} completada")
            ...
            # Persistir error
            self._persist_task_result(workflow.workflow_id, task.task_id, {"error": error_msg}, "failed")
            self._persist_log(workflow.workflow_id, error_msg, level="error")
    ...
    def _persist_workflow(self, workflow: WorkflowState, task_definitions: List[TaskDefinition]) -> None:
    ...
    def _update_workflow_status(self, workflow: WorkflowState) -> None:
    ...
    def _persist_task_result(self, workflow_id: str, task_id: str, result: Dict[str, Any], status: str) -> None:
    ...
    def _persist_workflow_result(self, workflow: WorkflowState) -> None:
    ...
    def _persist_log(self, workflow_id: str, message: str, level: str = "info", source: str = "orchestrator") -> None:
