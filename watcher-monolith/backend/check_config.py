...
print("1️⃣  Verificando OpenAI API Key...")

try:
    from app.core.agent_config import DEFAULT_AGENT_CONFIG
    
    api_key_from_config = DEFAULT_AGENT_CONFIG.openai_api_key
    ...
    if api_key_from_env:
        ...
        print(f"      Para habilitar OpenAI:")
        print(f"      - Opción 1: Crear archivo .env con OPENAI_API_KEY=tu-key")
        print(f"      - Opción 2: Ya está configurada en agent_config.py")
...
try:
    ...
    if agent.client:
        print("   ✅ Insight Agent inicializado con OpenAI client")
    else:
        print("   ⚠️  Insight Agent inicializado en modo fallback (sin OpenAI)")
...
print("\n💡 Notas:")
print("   • El chat funcionará con o sin API key de OpenAI")
print("   • Sin API key: respuestas fallback informativas")
print("   • Con API key: respuestas generadas por GPT-3.5")
print()
