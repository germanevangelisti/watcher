          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex flex-col items-end gap-1">
                ...
                {boletin.origin && boletin.origin !== "downloaded" && (
                  <span className={cn(
                    "inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full font-medium",
                    boletin.origin === "uploaded" && "bg-amber-500/10 text-amber-600",
                    boletin.origin === "synced" && "bg-cyan-500/10 text-cyan-600"
                  )}>
                    {boletin.origin === "uploaded" ? <Upload className="h-2.5 w-2.5" /> : <RefreshCw className="h-2.5 w-2.5" />}
                    {boletin.origin === "uploaded" ? "Manual" : "Sync"}
                  </span>
                )}
              </div>
            </div>
