...
export function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${error ? "bg-danger" : "bg-success"}`} />
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                {isLoading ? (
                  ...
                  <span className="text-lg font-bold">{stats?.total_documents ?? 0}</span>
                )}
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Database className="h-4 w-4 text-muted-foreground" />
                  <span className="text-sm">Fragmentos Indexados</span>
                </div>
                {isLoading ? (
                  <Skeleton className="h-6 w-16" />
                ) : (
                  <span className="text-lg font-bold">{stats?.total_chunks ?? 0}</span>
                )}
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Network className="h-4 w-4 text-muted-foreground" />
                  <span className="text-sm">Entidades Extraídas</span>
                </div>
                {isLoading ? (
                  <Skeleton className="h-6 w-16" />
                ) : (
                  <span className="text-lg font-bold">{stats?.total_entities ?? 0}</span>
                )}
              </div>
            </div>
          </CardContent>
      ...
      {/* Transparency Map - Mapa del Estado */}
      <TransparencyMap />
    </div>
  )
}
