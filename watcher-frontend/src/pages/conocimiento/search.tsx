            ...
            {searchMutation.data.results.length === 0 ? (
              ...
              <div className="space-y-4">
                {searchMutation.data.results.map((result: SearchResult, idx: number) => {
                  ...
                  const fileName = result.file_name || result.metadata?.filename
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 space-y-2">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {result.page_numbers && result.page_numbers.length > 0 && (
                              <Badge variant="secondary" className="text-xs">
                                Pág. {result.page_numbers.join(", ")}
                              </Badge>
                            )}
                          </div>
                          {hasHighlight ? (
                            <p
                              className="text-sm leading-relaxed [&>mark]:bg-yellow-300/40 [&>mark]:text-foreground [&>mark]:px-0.5 [&>mark]:rounded"
                              dangerouslySetInnerHTML={{ __html: displayText }}
                            />
                          ) : (
                            <p className="text-sm leading-relaxed line-clamp-4">{displayText}</p>
                          )}
                          {result.metadata && (
                            <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                              ...
                              {(result.metadata.section || result.metadata.section_type) && (
                                <span>Secc. {result.metadata.section || result.metadata.section_type}</span>
                              )}
                        ...
                        {docId && (
                          <Button
                            ...
                            onClick={() => {
                              window.open(`/documentos/${docId}`, "_blank")
                            }}
                          >
