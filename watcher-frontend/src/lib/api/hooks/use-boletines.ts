...
export function useBoletines(filters?: BoletinesFilters) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (filters?.jurisdiccion_id !== undefined) {
        params.append("jurisdiccion_id", filters.jurisdiccion_id.toString())
      }
